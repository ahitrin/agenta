...
;; The following structure is used in `setting/*.edn` files:
...
;; It's created from `setting` structure and inherits many properties from `unit-types` section (also mentioned as `unit-def`).
...
(clerk/table {
  ...
  :rows [
    ...
    ["choose-xys"       ['GameMap 'setting '[unit]]                 "{xy -> unit}"]
